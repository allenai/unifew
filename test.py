...
class PTLWrapperModel(Model):
    def __init__(self, seed: int, model_type: str, k=1, question=None, ckpt_path=None, output_path=None, **kwargs):
        super().__init__()

        args = argparse.Namespace(
            seed=seed, model_type=model_type, question=question, batch_size=1,
            k=k, predictions_path=output_path, **kwargs)
    ...
    def fit_and_predict(
        ...
        if val_x is not None:
            ...
            val_ds = list(UnifewDataset.get_current_set_separate_train_test(
                val_x, target_x, val_y, None, metadata, self.tokenizer, self.max_len, self.question_str,
                subset='train', args=self.args))
            ...
            val_ds = []
        train_ds = list(UnifewDataset.get_current_set_separate_train_test(
            support_x, target_x, support_y, None, metadata, self.tokenizer, self.max_len, self.question_str,
            subset='train', args=self.args))
        test_ds = list(UnifewDataset.get_current_set_separate_train_test(
            support_x, target_x, support_y, None, metadata, self.tokenizer, self.max_len, self.question_str,
            subset='val', args=self.args))

        # data is already batched, so set batch_size=1
        train_loader = DataLoader(MetatestDataset(train_ds), batch_size=1,
                                  collate_fn=UnifewDataset.collate_fn)
        val_loader = DataLoader(MetatestDataset(val_ds), batch_size=1,
                                collate_fn=UnifewDataset.collate_fn) if val_ds else None
        test_loader = DataLoader(MetatestDataset(test_ds), batch_size=1,
                                 collate_fn=UnifewDataset.collate_fn)
